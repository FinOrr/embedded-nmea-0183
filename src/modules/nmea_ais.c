...
/**
 * @brief Parse ABK sentence - AIS Addressed and Binary Broadcast Acknowledgement
 *
 ...
 *
 * Format: $--ACA,x,llll.ll,a,yyyyy.yy,a,llll.ll,a,yyyyy.yy,a,x,xxxx,x,xxxx,x,x,x,a,x,hhmmss.ss*hh<CR><LF>
 * Fields:
...
nmea_result_t nmea_parse_aca(nmea_context_t *ctx, const nmea_tokens_t *tokens)
{
    ...
    if (tokens->token_count > 3) {
        nmea_parse_coordinate(tokens->tokens[2], tokens->tokens[3], &ais->aca_ne_latitude);
    }
    if (tokens->token_count > 5) {
        nmea_parse_coordinate(tokens->tokens[4], tokens->tokens[5], &ais->aca_ne_longitude);
    }

    /* Parse SW corner coordinates (fields 6-9) */
    if (tokens->token_count > 7) {
        nmea_parse_coordinate(tokens->tokens[6], tokens->tokens[7], &ais->aca_sw_latitude);
    }
    if (tokens->token_count > 9) {
        nmea_parse_coordinate(tokens->tokens[8], tokens->tokens[9], &ais->aca_sw_longitude);
    }
...
nmea_result_t nmea_parse_lrf(nmea_context_t *ctx, const nmea_tokens_t *tokens)
{
    ...
    if (tokens->token_count > 3 && !nmea_is_empty_token(tokens->tokens[3])) {
        strncpy(ais->lrf_requestor_name, tokens->tokens[3], sizeof(ais->lrf_requestor_name) - 1);
        ais->lrf_requestor_name[sizeof(ais->lrf_requestor_name) - 1] = '\0';
    }

    /* Parse function request (field 4) */
    if (tokens->token_count > 4 && !nmea_is_empty_token(tokens->tokens[4])) {
        strncpy(ais->lrf_function_request, tokens->tokens[4], sizeof(ais->lrf_function_request) - 1);
 ...
 *
 * Format: $--LRI,x,a,xxxxxxxxx,xxxxxxxxx,llll.ll,a,yyyyy.yy,a,llll.ll,a,yyyyy.yy,a*hh<CR><LF>
 * Fields:
 *   1: Sequence number, 0-9
 *   2: Control flag
 *   3: MMSI of requestor
...
nmea_result_t nmea_parse_lri(nmea_context_t *ctx, const nmea_tokens_t *tokens)
{
    ...
    if (tokens->token_count > 6) {
        nmea_parse_coordinate(tokens->tokens[5], tokens->tokens[6], &ais->lri_ne_latitude);
    }
    if (tokens->token_count > 8) {
        nmea_parse_coordinate(tokens->tokens[7], tokens->tokens[8], &ais->lri_ne_longitude);
    }

    /* Parse SW corner coordinates (fields 9-12) */
    if (tokens->token_count > 10) {
        nmea_parse_coordinate(tokens->tokens[9], tokens->tokens[10], &ais->lri_sw_latitude);
    }
    if (tokens->token_count > 12) {
        nmea_parse_coordinate(tokens->tokens[11], tokens->tokens[12], &ais->lri_sw_longitude);
        ais->lri_valid = true;
    }
...
nmea_result_t nmea_parse_ssd(nmea_context_t *ctx, const nmea_tokens_t *tokens)
{
    ...
    if (tokens->token_count > 1 && !nmea_is_empty_token(tokens->tokens[1])) {
        strncpy(ais->ssd_callsign, tokens->tokens[1], sizeof(ais->ssd_callsign) - 1);
...
nmea_result_t nmea_parse_vsd(nmea_context_t *ctx, const nmea_tokens_t *tokens)
{
    ...
    if (tokens->token_count > 4 && !nmea_is_empty_token(tokens->tokens[4])) {
        strncpy(ais->vsd_destination, tokens->tokens[4], sizeof(ais->vsd_destination) - 1);
