 ...
 *
 * Implements zero-allocation tokenization using in-place null terminator
 * insertion.
 */
 ...
 * The buffer is modified in-place, so pass a copy if you need to preserve the
 * original.
 *
...
nmea_result_t nmea_tokenize(char* buffer, size_t buffer_size,
  ...
  *end = '\0';
  ...
  /* Do NOT restore sentinel - leave the null terminator in place so tokens are
   * properly terminated */
