 ...
 *
 * @note This header is automatically included by nmea.h when NMEA_MODULE_GNSS_ENABLED is set.
 */
...
typedef struct {
    ...
    char mode_indicators[8];                /**< Multi-system mode indicators (GNS: GPS/GLONASS/Galileo) */
    char nav_status;                        /**< Navigational status (S=Safe, C=Caution, U=Unsafe, V=Not valid) */
    ...
    uint8_t satellite_prns[NMEA_MAX_SATELLITES_USED]; /**< PRNs of satellites used in fix */
    nmea_satellite_t satellites[NMEA_MAX_SATELLITES]; /**< Detailed satellite info */
    ...
    float horizontal_protection_level;      /**< Horizontal protection level (meters) */
