...
void error_tracker_print_stats(error_tracker_t* tracker) {
  uint32_t total_errors = tracker->checksum_errors + tracker->syntax_errors +
                          tracker->semantic_errors + tracker->buffer_errors +
                          tracker->config_errors + tracker->state_errors +
  ...
  printf("Successful parses:   %u (%.1f%%)\n", tracker->successful_parses,
         tracker->total_sentences > 0 ? (float)tracker->successful_parses /
...
uint32_t get_system_time_ms(void) { return g_system_time_ms++; }

/**
 * Comprehensive error callback
 */
void nmea_error_callback(nmea_error_type_t error_type, nmea_result_t error_code,
  ...
  if ((now - tracker->last_log_time_ms) >= tracker->log_interval_ms) {
    fprintf(stderr, "[ERROR] Type: %d, Code: %s, Message: %s\n", error_type,
            nmea_get_error_string(error_code), message ? message : "N/A");
 ...
 */
nmea_result_t parse_with_error_handling(nmea_context_t* ctx,
  ...
  nmea_result_t result =
      nmea_parse(ctx, sentence, length, parse_buffer, buffer_size);
  ...
  switch (result) {
    ...
    case NMEA_ERROR_CHECKSUM_FAILED:
      ...
      if (tracker->consecutive_errors > 5) {
        fprintf(stderr,
                "Warning: %u consecutive checksum errors "
                "(check connections)\n",
                tracker->consecutive_errors);
      }
      break;
...
void example_basic_error_handling(void) {
  ...
  for (int i = 0; test_sentences[i] != NULL; i++) {
    ...
    nmea_result_t result = parse_with_error_handling(
        &ctx, &tracker, test_sentences[i], strlen(test_sentences[i]),
...
void example_error_recovery(void) {
  ...
  const char* bad_sentence =
      "$GPGGA,123519,4807.038,N,01131.000,E,1,08,0.9,545.4,M,46.9,M,,*FF\r\n";
  const char* good_sentence =
      "$GPGGA,123519,4807.038,N,01131.000,E,1,08,0.9,545.4,M,46.9,M,,*47\r\n";

  /* 5 bad sentences */
  for (int i = 0; i < 5; i++) {
    parse_with_error_handling(&ctx, &tracker, bad_sentence,
                              strlen(bad_sentence), parse_buffer,
                              sizeof(parse_buffer));
  }
  ...
  for (int i = 0; i < 3; i++) {
    parse_with_error_handling(&ctx, &tracker, good_sentence,
                              strlen(good_sentence), parse_buffer,
                              sizeof(parse_buffer));
  }
