...
int main(void) {
  ...
  const char* gps_sentences[] = {
      ...
      "$GPRMC,123519,A,4807.038,N,01131.000,E,022.4,084.4,100325,003.1,W,A*"
      "0D\r\n",
  ...
  for (int i = 0; gps_sentences[i] != NULL; i++) {
    nmea_result_t result =
        nmea_parse(&ctx, gps_sentences[i], strlen(gps_sentences[i]),
  ...
  if (gps_data.utc_time.valid) {
    ...
    printf("  %02d:%02d:%02d\n", gps_data.utc_time.hour,
           gps_data.utc_time.minute, gps_data.utc_time.second);
  }
  ...
  if (gps_data.utc_date.valid) {
    ...
    printf("  %04d-%02d-%02d\n", gps_data.utc_date.year,
           gps_data.utc_date.month, gps_data.utc_date.day);
  }
  ...
  if (gps_data.dop.valid) {
    ...
    if (gps_data.dop.hdop < 1.0)
      printf("Excellent");
    else if (gps_data.dop.hdop < 2.0)
      printf("Good");
    else if (gps_data.dop.hdop < 5.0)
      printf("Moderate");
    else
      printf("Poor");
    printf(")\n");
